...
describe('DiceRoller', () => {
    ...
    it('should evaluate simple math', () => {
        const result = DiceRoller.roll('1 + 1', mockContext);
        expect(result.total).toBe(2);
    });

    it('should replace stats with values', () => {
        const result = DiceRoller.roll('{Body} + 5', mockContext);
        expect(result.total).toBe(10); // 5 + 5
    });

    it('should replace derived stats', () => {
        const result = DiceRoller.roll('{Attack} + 2', mockContext);
    ...
    it('should roll dice (mock check)', () => {
        ...
        const result = DiceRoller.roll('1d6', mockContext);
    ...
    it('should handle complex formulas', () => {
        const result = DiceRoller.roll('2d6 + {Body}', mockContext);
    ...
    it('should detect criticals (max roll)', () => {
        ...
        const result = DiceRoller.roll('1d1', mockContext); // Always 1
    ...
    it('should detect fumbles (min roll)', () => {
        ...
        // Both true.
        const result = DiceRoller.roll('1d1', mockContext);
